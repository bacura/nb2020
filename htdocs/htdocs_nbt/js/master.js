...
var initMemory = function(){
	closeBroseWindows( 0 );
	$.post( "gm-memory.cgi", { command:'init' }, function( data ){ $( "#LF" ).html( data );});
	document.getElementById( "LF" ).style.display = 'block';
};

// List each pointer
var listPointer = function( category ){
	$.post( "gm-memory.cgi", { command:'list_pointer', category:category }, function( data ){ $( "#LF" ).html( data );});
};

// New category form
var newCategory = function(){
	$.post( "gm-memory.cgi", { command:'new_category' }, function( data ){ $( "#LF" ).html( data );});
};

// Save New category
var saveCategory = function(){
	var category = document.getElementById( 'category' ).value;
	$.post( "gm-memory.cgi", { command:'save_category', category:category }, function( data ){ $( "#LF" ).html( data );});
};

// Delete category
var deleteCategory = function( category, delete_check_no ){
	if( document.getElementById( delete_check_no ).checked ){
		$.post( "gm-memory.cgi", { command:'delete_category', category:category }, function( data ){ $( "#LF" ).html( data );});
	}else{
		displayVideo( 'Check!' );
	}
};
...
var newMemoryBat = function(){
	$.post( "gm-memory.cgi", { command:'new_bat' }, function( data ){ $( "#LF" ).html( data );});
};

// Save memory bat
var saveMemoryBat = function( mode ){
	var memory = document.getElementById( 'memory' ).value;
	$.post( "gm-memory.cgi", { command:'save_bat', memory:memory, mode:mode }, function( data ){ $( "#LF" ).html( data );});
	displayVideo( 'Saved' );
};
...
var savePMemory = function( category, post_process ){
	...
	if( post_process == 'front'){
		$.post( "gm-memory.cgi", { command:'save_pointer', memory:memory, category:category, pointer:pointer, rank:rank }, function( data ){ $( "#LF" ).html( data );});
	}else{
...
var movePMemory = function( category, pointer, post_process ){
	...
	if( post_process == 'front'){
		$.post( "gm-memory.cgi", { command:'move_pointer', memory:memory, category:category, pointer:pointer, rank:rank, mvcategory:mvcategory }, function( data ){ $( "#LF" ).html( data );});
	}else{
...
var deletePMemory = function( category, pointer, post_process ){
	if( document.getElementById( 'deletepm_check' ).checked ){
		if( post_process == 'front'){
			$.post( "gm-memory.cgi", { command:'delete_pointer', category:category, pointer:pointer, post_process }, function( data ){ $( "#LF" ).html( data );});
		}else{

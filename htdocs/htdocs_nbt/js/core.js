...
var bookOpen = function( url, level ){
	...
	$.ajax({ url:url, type:'GET', dataType:'html', success:function( data ){ $( "#L" + level ).html( data ); }});
	document.getElementById( "L" + level ).style.display = 'block';
};
...
var memoryOpen = function( com, category, pointer, depth ){
	...
	$.post( "memory.cgi", { command:com, category:category, pointer:pointer, depth:depth }, function( data ){ $( "#L" + depth ).html( data );});
...
var initMemoryPB = function(){
	closeBroseWindows( 3 );
	$.post( "memorypb.cgi", { command:"init" }, function( data ){ $( "#L3" ).html( data );});
	document.getElementById( "L3" ).style.display = 'block';
};
...
var nextMemoryPB = function(){
	...
	$.post( "memorypb.cgi", { command:"category", category:category, open_res:open_res }, function( data ){ $( "#L4" ).html( data );});
	document.getElementById( "L4" ).style.display = 'block';
};

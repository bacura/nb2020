#! /usr/bin/ruby
#encoding: utf-8
#Nutrition browser 2020 nutrition assessment tools 0.10b (2024/04/09)
...
#### Menu no line
def menu( user )
	mods = Dir.glob( "#{$HTDOCS_PATH}/ginmi_/mod_*" )
	mods.map! do |x|
		x = File.basename( x )
		x = x.sub( 'mod_', '' )
		x = x.sub( '.rb', '' )
	...
	mods.each.with_index( 1 ) do |e, i|
		require "#{$HTDOCS_PATH}/ginmi_/mod_#{e}.rb"
...
if mod == 'menu'
	...
	if mod == ''
		html =  "<div align='center'>アセスメント</div>"
	else

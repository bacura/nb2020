#! /usr/bin/ruby
#encoding: utf-8
#Nutrition browser GM food alias dictionary editor 0.5.0.AI (2024/10/14)
...
var changeDic = ( sg ) => {
	postReq( 'change', { sg }, data => $( "#L1" ).html( data ));
};

// Direct food name dictionary button
var saveDic = ( org_name_def, sg ) => {
	const aliases = $( '#' + org_name_def ).val();
	const dfn = $( '#dfn_' + org_name_def ).val();
	postReq( 'update', { org_name_def, aliases, sg, dfn }, data => {
		$( "#L1" ).html( data );
		displayREC();
	});
};
...
var newDic = () => {
	const org_name = $( '#new_org_name' ).val();
	const aliases = $( '#new_alias' ).val();
	const sg = $( '#new_fg' ).val();
	const dfn = $( '#dic_def_fn' ).val();
	postReq( 'new', { org_name, aliases, sg, dfn }, data => {
		$( "#L1" ).html( data );
		displayREC();
	});
};	

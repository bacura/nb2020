#! /usr/bin/ruby
#encoding: utf-8
#Nutrition browser 2020 GM memory editor 0.06b (2024/02/19)
...
var photoUpload = function( origin ){
...
var photoMove = function( code, zidx ){
	...
	$.post( "master/gm-memory.cgi", { command:'photo_mv', origin:'#{code}', code:code, zidx:zidx, base:base }, function( data ){ $( layer ).html( data );});
};

var photoDel = function( code ){
	$.post( "master/gm-memory.cgi", { command:'photo_del', origin:'#{code}', code:code, base:base }, function( data ){ $( layer ).html( data );});
};

#! /usr/bin/ruby
#encoding: utf-8
#Nutrition browser 2020 food ranking 0.11b (2024/02/04)
...
if command == 'list'
	...
	if fg == 0
		query = "SELECT FN, #{main_item} #{comp_sql} FROM #{$MYSQL_TB_FCT};"
	else
		sql_fg = fg
		sql_fg = "0#{fg}" if sql_fg < 10
		query = "SELECT FN, #{main_item} #{comp_sql} FROM #{$MYSQL_TB_FCT} WHERE FG='#{sql_fg}';"
	end
	...
	food_tag = Hash.new
	rr = db.query( "SELECT * FROM #{$MYSQL_TB_TAG};", false )
	rr.each do |rr| food_tag[rr['FN']] = rr end

	recipei = Hash.new
	rr = db.query( "SELECT word FROM #{$MYSQL_TB_RECIPEI} WHERE user='#{user.name}' OR public=1;", false )
...
@category.each.with_index do |e, i|
	...
	s = 'SELECTED' if i == fg
	if i == 0
		fg_select << "<option value='#{i}' #{s}>#{l['fg_all']}</option>"
	elsif i == 19
	else
		fg_select << "<option value='#{i}' #{s}>#{e}</option>"
	end
end
fg_select << '</select>'

...
var recipeEdit = function( com, code ){
	...
	$.post( "photo.cgi", { command:'form', code:code }, function( data ){ $( "L3" ).html( data );});
...
var recipeSave = function( code ){
	...
	if( recipe_name == '' ){
		...
		var fx = function(){
			...
			$.post( "photo.cgi", { command:'form', code:'' }, function( data ){ $( "L3" ).html( data );});
		};
		setTimeout( fx , 1000 );
	}
};
...
var photoSave = function( slot, code ){
	$form = $( '#photo_form' );
...
var photoDel = function( slot, code ){
	$.post( "photo.cgi", { command:'delete', code:code, slot:slot }, function( data ){ displayVIDEO( '写真を削除' );});
//	$.post( "photo.cgi", { command:'delete', code:code, slot:slot }, function( data ){ $( 'L3' ).html( data );});
	var fx = function(){
		$.post( 'photo.cgi', { command:'form', code:code }, function( data ){ $( 'L3' ).html( data );});
	};
	setTimeout( fx, 1000 );
...
var luckyAnalyze = function( mode ){
...
//	$.post( "lucky.cgi", { command:'analyze', mode:mode, lucky_data:lucky_data }, function( data ){});
	document.getElementById( "L2" ).style.display = 'block';
	$.post( "lucky.cgi", { command:'analyze', mode:mode, lucky_data:lucky_data }, function( data ){ $( "#L2" ).html( data );});
	document.getElementById( "L1" ).style.display = 'block';
...
var Pseudo_R2F = function( code ){
	closeBroseWindows( 3 );
	$.post( "pseudo_r2f.cgi", { command:'form', code:code }, function( data ){ $( "L3" ).html( data );});
	document.getElementById( "L3" ).style.display = 'block';
	L1 = 3;
};

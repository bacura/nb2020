...
<h4>使用データ</h4>
<p>　食品成分表ブラウザで使用する食品成分データは、文部科学省が公開している「日本食品標準成分表2020年版（八訂）増補2023年」を使用しています。<br>
データ中の()、Tr、-記号なども同様の意味で用いられています。また、当サイトで得たデータの使用については文部科学省の方針に従ってください。<br>
一部の食品の区分や名称は元データがカオスなので、独自に変更を加えています。<br>
詳しくは、<a href="http://www.mext.go.jp/a_menu/syokuhinseibun/index.htm">文部科学省　日本食品標準成分表・資源に関する取組</a>をご覧下さい。</p>

<p>　一部の食品重量の単位変換は女子栄養大学出版部出版の「調理のためのベーシックデータ第4版、第6版」を参考にしています。</p>

<h4>ローカルでの使用</h4>
<p>　栄養ブラウザは<a href='https://github.com/bacura/nb2020/'>Github</a>で全てのソースが公開されています。
その気になればローカルでサービス自体を稼働させることが出来ますがインストーラー、ドキュメント類が整備されておりません。
動かしてみたい人は吉山（ばきゅら京都Lab）info@bacura.jpまでご連絡ください。</p>
